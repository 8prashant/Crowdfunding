...
} from "./chunk-TTCWRZBM.js";
import {
  WagmiConnector
} from "./chunk-BNFA5NLB.js";
import {
  walletIds
...
var u32, createView, rotr, isLE, hexes, Hash, toStr;
var init_utils = __esm({
  "node_modules/@noble/hashes/esm/utils.js"() {
    init_shim();
    init_crypto();
    u32 = (arr2) => new Uint32Array(arr2.buffer, arr2.byteOffset, Math.floor(arr2.byteLength / 4));
    createView = (arr2) => new DataView(arr2.buffer, arr2.byteOffset, arr2.byteLength);
...
var _0n, _1n, _2n, hexes2, asciis, bitSet, bitMask, u8n, u8fr, validatorFns;
...
var require_lib = __commonJS({
  "node_modules/borsh/lib/index.js"(exports) {
    ...
    function baseEncode(value) {
      ...
      return bs58_1.default.encode(import_buffer.Buffer.from(value));
    }
    exports.baseEncode = baseEncode;
    function baseDecode(value) {
      return import_buffer.Buffer.from(bs58_1.default.decode(value));
    }
    ...
    var BinaryWriter = class {
      constructor() {
        this.buf = import_buffer.Buffer.alloc(INITIAL_LENGTH);
        this.length = 0;
      }
      maybeResize() {
        if (this.buf.length < 16 + this.length) {
          this.buf = import_buffer.Buffer.concat([this.buf, import_buffer.Buffer.alloc(INITIAL_LENGTH)]);
        }
      }
      writeU8(value) {
        this.maybeResize();
      ...
      writeU64(value) {
        this.maybeResize();
        this.writeBuffer(import_buffer.Buffer.from(new bn_js_1.default(value).toArray("le", 8)));
      }
      writeU128(value) {
        this.maybeResize();
        this.writeBuffer(import_buffer.Buffer.from(new bn_js_1.default(value).toArray("le", 16)));
      }
      writeU256(value) {
        this.maybeResize();
        this.writeBuffer(import_buffer.Buffer.from(new bn_js_1.default(value).toArray("le", 32)));
      }
      writeU512(value) {
        this.maybeResize();
        this.writeBuffer(import_buffer.Buffer.from(new bn_js_1.default(value).toArray("le", 64)));
      }
      writeBuffer(buffer) {
        this.buf = import_buffer.Buffer.concat([
          import_buffer.Buffer.from(this.buf.subarray(0, this.length)),
          buffer,
          import_buffer.Buffer.alloc(INITIAL_LENGTH)
        ]);
        this.length += buffer.length;
      }
      writeString(str) {
        this.maybeResize();
        const b = import_buffer.Buffer.from(str, "utf8");
        this.writeU32(b.length);
        this.writeBuffer(b);
      }
      writeFixedArray(array2) {
        this.writeBuffer(import_buffer.Buffer.from(array2));
      }
...
var require_browser = __commonJS({
  "node_modules/bigint-buffer/dist/browser.js"(exports) {
    ...
    function toBigIntLE2(buf) {
      {
        const reversed = import_buffer.Buffer.from(buf);
        reversed.reverse();
    ...
    function toBufferLE2(num, width) {
      {
        const hex = num.toString(16);
        const buffer = import_buffer.Buffer.from(hex.padStart(width * 2, "0").slice(0, width * 2), "hex");
        buffer.reverse();
        return buffer;
      }
      return converter.fromBigInt(num, import_buffer.Buffer.allocUnsafe(width), false);
    }
    exports.toBufferLE = toBufferLE2;
    function toBufferBE(num, width) {
      {
        const hex = num.toString(16);
        return import_buffer.Buffer.from(hex.padStart(width * 2, "0").slice(0, width * 2), "hex");
      }
      return converter.fromBigInt(num, import_buffer.Buffer.allocUnsafe(width), true);
    }
...
var init_index_browser_esm = __esm({
  "node_modules/@solana/web3.js/lib/index.browser.esm.js"() {
    ...
    toBuffer = (arr2) => {
      if (import_buffer2.Buffer.isBuffer(arr2)) {
        return arr2;
      } else if (arr2 instanceof Uint8Array) {
        return import_buffer2.Buffer.from(arr2.buffer, arr2.byteOffset, arr2.byteLength);
      } else {
        return import_buffer2.Buffer.from(arr2);
      }
    };
    Struct2 = class {
    ...
    bigInt = (length) => (property) => {
      ...
      bigIntLayout.decode = (buffer, offset2) => {
        ...
        return (0, import_bigint_buffer.toBigIntLE)(import_buffer2.Buffer.from(src2));
      };
...
var global$1 = typeof _global !== "undefined" ? _global : typeof self !== "undefined" ? self : typeof window !== "undefined" ? window : {};
var lookup = [];
var revLookup = [];
...
function read(buffer, offset2, isLE2, mLen, nBytes) {
  var e, m;
...
function write(buffer, value, offset2, isLE2, mLen, nBytes) {
  var e, m, c;
  var eLen = nBytes * 8 - mLen - 1;
  var eMax = (1 << eLen) - 1;
  var eBias = eMax >> 1;
  ...
  enable(email) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      if (existedSDK && existedSDK.isBlocto) {
        ...
        return new Promise((resolve, reject) => setTimeout(() => existedSDK.enable().then(resolve).catch(reject), 10));
      }
...
var AptosProvider = class extends BloctoProvider {
  ...
  connect() {
    return __awaiter(this, void 0, void 0, function* () {
      const existedSDK = window.bloctoAptos;
      if (existedSDK) {
        return new Promise((resolve, reject) => setTimeout(() => existedSDK.connect().then(resolve).catch(reject), 10));
      }
      return new Promise((resolve, reject) => {
        var _a, _b;
        if (typeof window === "undefined") {
...
var BloctoConnector = class extends WagmiConnector {
  ...
  getProvider() {
    ...
    if (!_classPrivateFieldGet(this, _provider)) {
      const _chainId = (_c = (_b = chainId != null ? chainId : this.options.chainId) != null ? _b : (_a = this.chains[0]) == null ? void 0 : _a.chainId) != null ? _c : 1;
      const _rpc = (_d = this.chains.find((x) => x.chainId === _chainId)) == null ? void 0 : _d.rpc[0];
      _classPrivateFieldSet(this, _provider, (_e = new BloctoSDK({
        ethereum: {
          chainId: _chainId,
          rpc: _rpc
        },
        appId: this.options.appId
      })) == null ? void 0 : _e.ethereum);
    }
    if (!_classPrivateFieldGet(this, _provider)) {
 ...
 */
//# sourceMappingURL=thirdweb-dev-wallets-evm-connectors-blocto.browser.esm-UTCDA64T.js.map
